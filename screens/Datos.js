...
// =========================================================
// ✅ CORRECCIÓN 1: Importar 'auth' para obtener el UID del usuario actual
...
export default function Datos({ route, navigation }) {
    // =========================================================
    // ✅ CORRECCIÓN 2: Acceso seguro al userId
    // 1. Obtener de parámetros de forma segura (fallback {} si route.params es undefined)
    const { userId: routeUserId } = route.params || {};
    
    // 2. Usar el ID del usuario autenticado como fuente de verdad (más robusto que el nav param)
    const authenticatedUserId = auth.currentUser?.uid;
    
    // 3. Determinar el ID final (se prefiere el autenticado, si no, el del parámetro)
    ...
    const opcionesLimitaciones = [
        ...
        'Tobillos',
        'Movilidad Reducida',
        'Otro' // Se añade para manejar el input manual
    ];
    ...
    // =========================================================
    // ✅ CORRECCIÓN 3: Validación del userId al montar el componente
    // (Si no hay ID, redirige inmediatamente para evitar el crash)
    useEffect(() => {
        if (!userId) {
            Alert.alert(
                "Error de Sesión", 
        ...
        fetchUserData();
    }, [userId]); // Ahora depende del userId calculado
    // =========================================================
    ...
    const guardarDatos = async () => {
        ...
        // Validar si tenemos ID antes de guardar
        if (!userId) {
            Alert.alert("Error de Sesión", "Falta el ID del usuario para guardar.");
            return;
        }
    ...
    const renderCheckboxGroupWithManualInput = (
        ...
        // Asegura que 'Otro' esté disponible sin duplicados
                ...
                <View style={styles.checkboxContainerTidy}>
                    {opciones.map((opcion) => {
                        ...
                        return (
                            <TouchableOpacity
                                key={opcion}
                                style={[styles.checkboxChipTidy, sel && styles.checkboxChipSelected]}
    ...
    // Si el ID es nulo y la carga aún no ha terminado, mostramos la actividad para evitar un flash en el error
    if (!userId && !loading) {
        // El useEffect de arriba debería redirigir, pero si el flujo es muy rápido:
        return (
            <View style={[styles.mainContainer, { justifyContent: 'center', alignItems: 'center' }]}>
                <ActivityIndicator size="large" color={COLORS.brandGreenDark} />
                <Text style={{color: COLORS.textMedium, marginTop: 10}}>Verificando estado de sesión...</Text>
            </View>
        );
    }
        ...
        <View style={styles.mainContainer}>
            ...
            <KeyboardAvoidingView style={{ flex: 1 }}>
                <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
                    ...
                    {/* BOTÓN GUARDAR */}
                    <TouchableOpacity 
                        style={[styles.saveButton, loading && styles.botonDeshabilitado]} 
                        onPress={guardarDatos} 
                        disabled={loading || !userId} // Deshabilitar si está cargando o falta el ID
                    >
                        {loading ? (
...
const styles = StyleSheet.create({
    ...
    saveButtonText: { color: '#FFFFFF', fontSize: 18, fontWeight: '900', letterSpacing: 0.5 },
});